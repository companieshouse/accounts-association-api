...
public class UserCompanyAssociations implements UserCompanyAssociationsInterface {
    ...
    private final AssociationsService assoctiationService;

    @Autowired
    public UserCompanyAssociations(final CompanyService companyService, final TransactionalService transactionalService, final UsersService usersService, final EmailService emailService, final AssociationsService assoctiationService) {
        this.companyService = companyService;
        this.transactionalService = transactionalService;
        this.usersService = usersService;
        this.emailService = emailService;
        this.assoctiationService = assoctiationService;
    }
    ...
    public ResponseEntity<ResponseBodyPost> addAssociation(final RequestBodyPost requestBody) {
        ...
        final var targetAssociationDao = assoctiationService.fetchAssociationDao(targetUser, companyDetails);

        emailService.sendAddAssociationStatusUpdateEmails(targetUser, companyDetails, targetAssociationDao);

        return new ResponseEntity<>(new ResponseBodyPost().associationLink(String.format("/associations/%s", targetAssociationDao.getId())), CREATED);
    }
    ...
    public ResponseEntity<AssociationsList> fetchAssociationsBy(final List<String> status, final Integer pageIndex, final Integer itemsPerPage, final String companyNumber) {
        ...
        final var associationsList = transactionalService.fetchAssociationsForUserAndPartialCompanyNumberAndStatuses(getUser(), companyNumber, new HashSet<>(status), pageIndex, itemsPerPage);
